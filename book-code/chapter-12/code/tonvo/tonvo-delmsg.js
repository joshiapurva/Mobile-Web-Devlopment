...
util.http_error = function( cb ) {
  return function(jqXHR, textStatus, errorThrown){
    ...
    util.log(err,jqXHR.status,textStatus,errorThrown)
...
function Twitter(reqtokens) {
  ...
  self.delmsg = function( account, msgid, cb ) {
    self.call(account,{
      path:self.conf.apibaseurl+'direct_messages/destroy/'+msgid+'.json',
      method:'DELETE',
      jsondata:true
    },function(err,data){
      if( err ) return cb(err,null);
...
function Social() {
  ...
  // delete a msg
  self.delmsg = function(account,msgid) {
    var network = self.network[account.network]

    network.delmsg( account, msgid, function(err,msgs) {
      if( err ) return util.log(err);
    })
  }
  ...
  self.clear = function( account ) {
    var acc = self.findaccount( account.nick, account.network )
    self.accounts.splice( acc.index$, 1 )
...
function Convo(social) {
  ...
  self.displaymsg = function( msg ) {
    ...
    if( !msg.f ) {
      item.swipe(function(){
        var remove = confirm('Delete this message?')

        if( remove ) {
          social.delmsg(self.account,msg.i)
          self.convo.splice(msg.index$,1)
          util.save('convo',social.convo)
          self.dirty = true
          self.render()
        }
      })
...
function Accounts(social) {
  ...
  self.render = function() {
    ...
    for( var i = 0; i < social.accounts.length; i++ ) {
      ...
      item.swipe(function(i){
        return function(){
          var account = social.accounts[i]

          var remove = confirm('Remove '+account.nick+' ('+account.network+') from this app?')

          if( remove ) {
          
          //navigator.notification.confirm(
          //  'Remove '+account.nick+' ('+account.network+') from this app?', 
          //  function(index){
          //    if( 0 == index ) {

            social.clear(account)
