...
var server = 'YOUR_IP_ADDRESS'

var s3prefix = 'https://s3.amazonaws.com/YOUR_S3_BUCKET_NAME/'
...
function App() {
  ...
  function showimages(images) {
    ...
    for( var i = images.length-1; 0 <= i; i-- ) {
      var whenstr = images[i].when.substring(11,16)
      var li = 
        lifestream_image.clone()        
        .css({display:'block'})
      li.find('span').text( images[i].user + ' ' + whenstr )
