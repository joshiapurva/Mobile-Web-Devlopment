...
function post(req,res) {
  ...
  finduser(true,['followers'],req,res,function(user,coll){
    append(username,username,picid,now,coll,merr,function(){
      common.sendjson(res,{ok:true})
      
      var followers = user.followers
      function appendfollower(fI) {
        if( fI < followers.length ) {
          append(followers[fI],username,picid,now,coll,merr,function(){
            appendfollower(fI+1)
          })
        }
      }
...
function append(touser,fromuser,picid,when,coll,merr,cb) {
  coll.findOne(
    {username:touser},
    {fields:['stream']},
    ...
    merr(function(user){
      ...
      coll.update(
        {username:touser},
        {$push:{stream:{picid:picid,user:fromuser,when:when}}},
        merr(function(){
...
function clienterror() {
  return function(req,res,next){
    if( '/lifestream/api/log/error?desc=' == req.url.substring(0,31) ) {
      res.writeHead(200)
      res.end()

      var desc = {}

      var report = JSON.parse(unescape(req.url.substring(31)))
      console.dir(report)
